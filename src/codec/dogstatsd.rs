...
fn encode(value: &Value) -> Result<Vec<u8>> {
    ...
    let val = value.get("value").ok_or(ErrorKind::InvalidDogStatsD)?;
    if !val.is_number() {
...
fn decode(data: &[u8], _ingest_ns: u64) -> Result<Value> {
    ...
    match d.next() {
        Some((i, b'c' | b'h' | b's' | b'd' )) => m.insert("type".into(), substr(data, i..=i)?.into()),
        Some((i, b'm')) => {
...
mod test {
    ...
    #[test]
    fn test_dogstatsd_distribtion() {
        let data = b"dogstatsd.distribution:123|d";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
            "metric": "dogstatsd.distribution",
            "value": 123,
        });
        assert_eq!(parsed, expected);
        let encoded = encode(&parsed).expect("failed to encode");
    ...
    #[test]
    fn test_dogstatsd_single_tag() {
        let data = b"dogstatsd.distribution:123|d|#env:dev";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
            "metric": "dogstatsd.distribution",
    ...
    #[test]
    fn test_dogstatsd_multiple_tags() {
        let data = b"dogstatsd.distribution:123|d|#env:dev,region:us-central1,rust";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
            "metric": "dogstatsd.distribution",
    ...
    #[test]
    fn test_dogstatsd_multiple_tags_trailing_comma() {
        let data = b"dogstatsd.distribution:123|d|#env:dev,region:us-central1,rust,,";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
    ...
    #[test]
    fn test_dogstatsd_container_id() {
        let data = b"dogstatsd.distribution:123|d|c:83c0a99c0a54c0c187f461c7980e9b57f3f6a8b0c918c8d93df19a9de6f3fe1d";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
            "metric": "dogstatsd.distribution",
            "value": 123,
            "container_id": "83c0a99c0a54c0c187f461c7980e9b57f3f6a8b0c918c8d93df19a9de6f3fe1d",
        });
        assert_eq!(parsed, expected);
        let encoded = encode(&parsed).expect("failed to encode");
        assert_eq!(encoded, data);
    ...
    #[test]
    fn test_dogstatsd_protocol_1_1() {
        let data = b"dogstatsd.distribution:123|d|@0.5|#env:dev,region:us-central1";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
            "metric": "dogstatsd.distribution",
    ...
    #[test]
    fn test_dogstatsd_protocol_1_2() {
        let data = b"dogstatsd.distribution:123|d|@0.5|#env:dev,region:us-central1|c:83c0a99c0a54c0c187f461c7980e9b57f3f6a8b0c918c8d93df19a9de6f3fe1d";
        let parsed = decode(data, 0).expect("failed to decode");
        let expected = literal!({
            "type": "d",
            "metric": "dogstatsd.distribution",
